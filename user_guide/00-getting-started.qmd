...
Use `connect()` to open a store that was previously created:
...
The embedding configuration is automatically restored from the stored metadata. Use `create()` for new stores and `connect()` for existing ones.
...
def search_docs(query: str) -> str:
    ...
    results = []
    for chunk in chunks:
        context = f" ({chunk.context})" if chunk.context else ""
        results.append(f"Source: {chunk.origin}{context}\n{chunk.text}")
    return "\n\n---\n\n".join(results)
