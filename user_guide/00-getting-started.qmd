...
The shared `ingest()` function handles reading, chunking, embedding, and storing documents in one step:
...
results = ingest(links, store=store, prepare=prepare, num_workers=4)
```
...
results = ingest(links, store=store, progress=True)
