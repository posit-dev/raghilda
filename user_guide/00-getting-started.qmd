...
**2. Chunk the document** — `MarkdownChunker` splits the Markdown into overlapping chunks at semantic boundaries (headings, paragraphs, sentences). The defaults are a chunk size of 1600 characters (~400 tokens) with 50% overlap between chunks. Each chunk retains the heading hierarchy it falls under as context.
