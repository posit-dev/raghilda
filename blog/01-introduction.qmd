...
date: "2026-03-13"
author: Daniel Falbel and Tomasz Kalinowski
---


We're happy to introduce Raghilda, a new Python package for building RAG (Retrieval-Augmented Generation) solutions.

RAG is a simple concept that comes up anytime you want to _retreive_ content for an LLM to improve or _augment_ the _generated_ output.

```{mermaid}
%%| label: fig-rag
%%| fig-cap: "Without RAG, the LLM generates a response using only the user query. With RAG, relevant documents are retrieved and provided to the LLM before it generates a response."
