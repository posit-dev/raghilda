from __future__ import annotations

from dataclasses import dataclass
import importlib
import json
import logging
import os
from pathlib import Path
from typing import Any, Optional, Sequence, Callable
from concurrent.futures import ThreadPoolExecutor

from ._store import BaseStore
from .chunk import MarkdownChunk, RetrievedChunk, Metric
from .chunker import MarkdownChunker
from .document import Document, MarkdownDocument
from .embedding import EmbeddingProvider, EmbedInputType, embedding_from_config
from .read import read_as_markdown
from tqdm import tqdm


logger = logging.getLogger(__name__)

_METADATA_TITLE_KEY = "raghilda_title"
_METADATA_EMBED_CONFIG_KEY = "raghilda_embed_config"


def _import_chromadb():
    try:
        chromadb = importlib.import_module("chromadb")
    except ModuleNotFoundError as exc:
        raise ModuleNotFoundError(
            "ChromaDB is required to use ChromaDBStore. Install with `pip install chromadb`."
        ) from exc
    return chromadb


def _get_client(location: str | Path | None):
    chromadb = _import_chromadb()
    if location is None or str(location) == ":memory:":
        return chromadb.Client()
    return chromadb.PersistentClient(path=str(location))


@dataclass
class ChromaDBMarkdownChunk(MarkdownChunk):
    """MarkdownChunk with ChromaDB-specific fields for storage."""

    doc_id: Optional[str] = None
    chunk_id: Optional[int] = None

    def __init__(
        self,
        text: str,
        start_index: int,
        end_index: int,
        context=None,
        token_count=None,
        doc_id=None,
        chunk_id=None,
    ):
        if token_count is None:
            token_count = len(text)

        super().__init__(
            text=text,
            start_index=start_index,
            end_index=end_index,
            token_count=token_count,
            context=context,
        )

        self.doc_id = doc_id
        self.chunk_id = chunk_id


@dataclass
class RetrievedChromaDBMarkdownChunk(ChromaDBMarkdownChunk, RetrievedChunk):
    """ChromaDBMarkdownChunk with retrieval metrics."""

    def __init__(
        self,
        text: str,
        start_index: int,
        end_index: int,
        context=None,
        token_count=None,
        doc_id=None,
        chunk_id=None,
        metrics=None,
    ):
        super().__init__(
            text=text,
            start_index=start_index,
            end_index=end_index,
            context=context,
            token_count=token_count,
            doc_id=doc_id,
            chunk_id=chunk_id,
        )

        if metrics is None:
            metrics = []
        self.metrics = metrics


@dataclass
class ChromaDBStoreMetadata:
    name: str
    title: str
    embed: Optional[EmbeddingProvider]


class ChromaDBStore(BaseStore):
    """A vector store backed by ChromaDB.

    ChromaDBStore provides local vector storage using Chroma's embedded client.
    Documents are chunked by raghilda and embeddings are generated by a
    raghilda EmbeddingProvider.

    Examples
    --------
    ```{python}
    #| eval: false
    from raghilda.store import ChromaDBStore
    from raghilda.embedding import EmbeddingOpenAI

    store = ChromaDBStore.create(
        location="raghilda_chroma",
        embed=EmbeddingOpenAI(),
        name="docs",
    )

    store.insert(markdown_doc)
    chunks = store.retrieve("hello world", top_k=3)
    ```
    """

    @staticmethod
    def create(
        location: str | Path | None = None,
        embed: Optional[EmbeddingProvider] = None,
        *,
        overwrite: bool = False,
        name: Optional[str] = None,
        title: Optional[str] = None,
        collection_metadata: Optional[dict[str, Any]] = None,
        client: Any = None,
    ):
        """Create a new ChromaDB store.

        Parameters
        ----------
        location
            Path where ChromaDB will persist its data. Use ":memory:" or None
            for an in-memory store.
        embed
            Embedding provider for generating vector embeddings.
        overwrite
            Whether to overwrite an existing collection with the same name.
        name
            Collection name for the store.
        title
            Human-readable title for the store.
        collection_metadata
            Additional metadata to attach to the Chroma collection.
        client
            Optional pre-configured Chroma client (e.g., HttpClient).

        Returns
        -------
        ChromaDBStore
            A newly created store instance.
        """
        if name is None:
            name = "raghilda_chroma"
        if title is None:
            title = "Raghilda ChromaDB Store"

        if client is None:
            client = _get_client(location)

        if overwrite:
            try:
                client.delete_collection(name=name)
            except Exception:
                pass

        store_metadata = {
            _METADATA_TITLE_KEY: title,
        }
        if embed is not None:
            store_metadata[_METADATA_EMBED_CONFIG_KEY] = json.dumps(embed.get_config())

        merged_metadata = dict(collection_metadata or {})
        merged_metadata.update(store_metadata)

        collection = client.create_collection(
            name=name,
            metadata=merged_metadata,
            embedding_function=None,
        )

        return ChromaDBStore(
            client=client,
            collection=collection,
            metadata=ChromaDBStoreMetadata(name=name, title=title, embed=embed),
        )

    @staticmethod
    def connect(
        name: str,
        location: str | Path | None = None,
        *,
        embed: Optional[EmbeddingProvider] = None,
        client: Any = None,
    ):
        """Connect to an existing ChromaDB store.

        Parameters
        ----------
        name
            Collection name for the store.
        location
            Path where ChromaDB persists its data. Use ":memory:" or None
            for an in-memory store.
        embed
            Optional embedding provider. If None, attempts to restore from
            collection metadata.
        client
            Optional pre-configured Chroma client (e.g., HttpClient).

        Returns
        -------
        ChromaDBStore
            A connected store instance.
        """
        if client is None:
            client = _get_client(location)

        collection = client.get_collection(name=name, embedding_function=None)
        metadata = collection.metadata or {}
        title = metadata.get(_METADATA_TITLE_KEY, "Raghilda ChromaDB Store")

        if embed is None:
            embed_config_json = metadata.get(_METADATA_EMBED_CONFIG_KEY)
            if embed_config_json:
                try:
                    embed = embedding_from_config(json.loads(embed_config_json))
                except (ValueError, json.JSONDecodeError) as exc:
                    logger.warning(
                        "Could not restore embedding provider from metadata: %s",
                        exc,
                    )

        return ChromaDBStore(
            client=client,
            collection=collection,
            metadata=ChromaDBStoreMetadata(name=name, title=title, embed=embed),
        )

    def __init__(self, client: Any, collection: Any, metadata: ChromaDBStoreMetadata):
        self.client = client
        self.collection = collection
        self.metadata = metadata

    def insert(self, document: Document) -> None:
        if not isinstance(document, MarkdownDocument):
            raise ValueError("Only MarkdownDocument is supported for ChromaDBStore")
        if document.chunks is None:
            raise ValueError("Document must be chunked before insertion")
        if self.metadata.embed is None:
            raise ValueError("No embedding provider configured for the store")

        texts = [chunk.text for chunk in document.chunks]
        embeddings = self.metadata.embed.embed(texts, EmbedInputType.DOCUMENT)

        ids = []
        metadatas = []
        for idx, chunk in enumerate(document.chunks):
            ids.append(f"{document.id}:{idx}")
            metadata = {
                "doc_id": document.id,
                "chunk_id": idx,
                "start_index": chunk.start_index,
                "end_index": chunk.end_index,
                "token_count": chunk.token_count,
                "context": chunk.context,
                "origin": document.origin,
            }
            metadatas.append({k: v for k, v in metadata.items() if v is not None})

        self.collection.upsert(
            ids=ids,
            documents=texts,
            embeddings=embeddings,
            metadatas=metadatas,
        )

    def ingest(
        self,
        uris: Sequence[str],
        prepare: Optional[Callable[[str], Document]] = None,
        num_workers: Optional[int] = None,
        progress=True,
    ) -> None:
        """
        Ingest multiple documents from a list of URIs.

        Parameters
        ----------
        uris
            A sequence of URIs (file paths, URLs, etc.) to ingest.
        prepare
            A callable that takes a URI and returns a MarkdownDocument with chunks computed.
        num_workers
            The number of worker threads to use for parallel ingestion. If None, defaults
            to the number of CPU cores.
        progress
            Whether to display a progress bar during ingestion. Default is True.
        """
        if num_workers is None:
            num_workers = os.cpu_count() or 1

        if prepare is None:
            chunker = MarkdownChunker()

            def _prepare(uri: str) -> Document:
                return chunker.chunk_document(read_as_markdown(uri))

            prepare = _prepare

        def do_ingest_work(uri: str) -> None:
            try:
                chunked_doc = prepare(uri)
                self.insert(chunked_doc)
            except Exception as e:
                raise RuntimeError(f"Failed to ingest '{uri}': {e}") from e

        with ThreadPoolExecutor(max_workers=num_workers) as pool:
            list(
                tqdm(
                    pool.map(do_ingest_work, uris),
                    total=len(uris),
                    disable=not progress,
                )
            )

    def retrieve(
        self, text: str, top_k: int, **kwargs
    ) -> Sequence[RetrievedChromaDBMarkdownChunk]:
        if self.metadata.embed is None:
            raise ValueError("No embedding provider configured for the store")

        query_embedding = self.metadata.embed.embed([text], EmbedInputType.QUERY)[0]

        results = self.collection.query(
            query_embeddings=[query_embedding],
            n_results=top_k,
            include=["documents", "metadatas", "distances"],
            **kwargs,
        )

        documents = (results.get("documents") or [[]])[0]
        metadatas = (results.get("metadatas") or [[]])[0]
        distances = (results.get("distances") or [[]])[0]

        output: list[RetrievedChromaDBMarkdownChunk] = []
        for doc_text, metadata, distance in zip(
            documents, metadatas, distances, strict=False
        ):
            metadata = metadata or {}
            start_index = int(metadata.get("start_index", 0))
            end_index = int(metadata.get("end_index", start_index + len(doc_text)))
            token_count = int(metadata.get("token_count", len(doc_text)))
            metrics = []
            if distance is not None:
                metrics.append(Metric(name="distance", value=distance))
            chunk = RetrievedChromaDBMarkdownChunk(
                text=doc_text,
                start_index=start_index,
                end_index=end_index,
                context=metadata.get("context"),
                token_count=token_count,
                doc_id=metadata.get("doc_id"),
                chunk_id=metadata.get("chunk_id"),
                metrics=metrics,
            )
            output.append(chunk)

        return output

    def size(self) -> int:
        results = self.collection.get(include=["metadatas"])
        metadatas = results.get("metadatas") or []
        doc_ids = {
            metadata.get("doc_id")
            for metadata in metadatas
            if metadata and metadata.get("doc_id")
        }
        return len(doc_ids)
